	...
	class direcciones {
		...
		public function set_sector($sector) {
			try {
				...
				// Validar la longitud y el formato del dato
				if (strlen($sector) > 80 || !preg_match('/^[\w\sàáâãéèêëìíîïòóôõùúûüÀÁÂÃÉÈÊËÌÍÎÏÒÓÔÕÙÚÛÜñÑª\!\"\'\-\#\&\(\)\,\.\;\¿\¡\!\?\`\°\/\:\|]+$/i', $sector)) {
		...
		public function set_calle($calle) {
			try {
				...
				// Validar la longitud y el formato del dato
				if (strlen($calle) > 80 || !preg_match('/^[\w\sàáâãéèêëìíîïòóôõùúûüÀÁÂÃÉÈÊËÌÍÎÏÒÓÔÕÙÚÛÜñÑª\!\"\'\-\#\&\(\)\,\.\;\¿\¡\!\?\`\°\/\:\|]+$/i', $calle)) {
		...
		public function set_nro_casa($nro_casa) {
			try {
				...
				// Validar la longitud y el formato del dato
				if (strlen($nro_casa) > 80 || !preg_match('/^[\w\sàáâãéèêëìíîïòóôõùúûüÀÁÂÃÉÈÊËÌÍÎÏÒÓÔÕÙÚÛÜñÑª\!\"\'\-\#\&\(\)\,\.\;\¿\¡\!\?\`\°\/\:\|]+$/i', $nro_casa)) {
		...
		public function set_punto_referencia($punto_referencia) {
			try {
				...
				// Validar la longitud y el formato del dato
				if (strlen($punto_referencia) > 300 || !preg_match('/^[\w\sàáâãéèêëìíîïòóôõùúûüÀÁÂÃÉÈÊËÌÍÎÏÒÓÔÕÙÚÛÜñÑª\!\"\'\-\#\&\(\)\,\.\;\¿\¡\!\?\`\°\/\:\|]+$/i', $punto_referencia)) {
