...
if (isset($_POST['cedula'],$_POST['clave']) and ($_POST['cedula'] != "" and $clave = $_POST['clave'] != "")) {
	...
	if ($consulta_persona = $conexion->query($sql)) {
		...
		if ($resultado_usuario == NULL) {
			...
			$bitacora->select = 'usuarios.idUsuarios,bitacora.fechaInicioSesion, bitacora.horaInicioSesion, bitacora.linksVisitados';
			$bitacora->from = 'bitacora inner join usuarios on (usuarios.idUsuarios = bitacora.idUsuarios)';
			$bitacora->condition = 'bitacora.horaFinalSesion = "NULL"';
			$bitacora->leer();
